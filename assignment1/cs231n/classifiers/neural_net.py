...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    Hid_act = ReLU(np.add(X.dot(W1),b1))
    scores  = np.add(Hid_act.dot(W2),b2) 
    ...
    los_fra = exp_fra[np.arange(N),y]
    
    # calculate the loss:
    loss = np.sum(-np.log(los_fra))
    
    # correct loss for average and regularization:
    # average
    loss /= N
    ...
    dL_dW2 = Hid_act.transpose().dot(dL_dsco)
    dL_db2 = np.sum(dL_dsco,0)
    dL_dHa = np.multiply(bin_act, dL_dsco.dot(W2.transpose()))
    dL_dW1 = X.transpose().dot(dL_dHa)
    dL_db1 = np.sum(dL_dHa,0)

    # correct gradient for average and regularization:
    # average
    dL_dW2   /= N 
    dL_db2   /= N    
    dL_dW1   /= N 
    ...
    # store the gradients in the grads directory
    grads['W2'] = dL_dW2
    grads['b2'] = dL_db2
    grads['W1'] = dL_dW1
    grads['b1'] = dL_db1
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      lrn_rt = learning_rate
    
      self.params['W1'] -= lrn_rt * grads['W1']
      self.params['b1'] -= lrn_rt * grads['b1']
      self.params['W2'] -= lrn_rt * grads['W2']
      self.params['b2'] -= lrn_rt * grads['b2']  
